// 航旅纵横二进制净化：Egern 原生入口。
// 内嵌算法快照来自 Umetrip.Clean.Loon.js（2026.09.11.4）。
export default async function(ctx) {
  if (ctx.response?.status !== 200) return;
  const originalResponseBody = new Uint8Array(await ctx.response.arrayBuffer());
  const headerRpid = ctx.request?.headers?.get?.('rpid') ??
    ctx.request?.headers?.rpid ?? ctx.request?.headers?.RPID;
  const $request = {
    url: ctx.request?.url ?? '',
    headers: headerRpid == null ? {} : {rpid: String(headerRpid)},
  };
  const $response = {status: ctx.response.status, body: originalResponseBody};
  ...
  return await new Promise((resolve) => {
    ...
    const $done = (result = {}) => {
      if (finished) return;
      finished = true;
      const body = result.bodyBytes ?? result.body ?? originalResponseBody;
      resolve({body: body instanceof ArrayBuffer ? new Uint8Array(body) : body});
    };

// 航旅纵横净化算法快照（源文件为 Loon 二进制接口适配）。
...
// 保留上游净化算法；兼容层将内部 $done 结果转换为 Egern 原生返回值。
